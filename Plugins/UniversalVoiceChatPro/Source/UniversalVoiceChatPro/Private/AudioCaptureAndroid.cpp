...
extern "C"
{
	JNIEXPORT void Java_com_epicgames_unreal_GameActivity_dataMicPayload(JNIEnv * jni, jclass clazz, jshortArray dataMic, jint readSize)
	{
		UE_LOG(LogTemp, Warning, TEXT("micro c++ Java_com_epicgames_unreal_GameActivity_dataMicPayload sizee %d"), (int) readSize);
		...
		if (readSize > 0) {
			...
			if (_microphoneSpeakComponentToCallback != NULL && _microphoneSpeakComponentToCallback->IsValidLowLevel() && IsValidChecked(_microphoneSpeakComponentToCallback)/*!_microphoneSpeakComponentToCallback->IsPendingKill()*/) {
