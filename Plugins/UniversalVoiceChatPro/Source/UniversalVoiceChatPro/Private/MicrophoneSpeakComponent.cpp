...
#if PLATFORM_WINDOWS
		UE_LOG(LogTemp, Warning, TEXT("UMicrophoneSpeakComponent::initAudioResources recreate Windows capture"));
...
bool UMicrophoneSpeakComponent::startSpeaking(bool _shouldHearMyOwnVoice, bool _isGlobal, int _radioChannel, bool _useRange, float _maxRange) {
	...
	if (!wasInitAudioResources) {
		...
		if (!resInit) {
			UE_LOG(LogTemp, Warning, TEXT("UMicrophoneSpeakComponent::startSpeaking wasn't init default"));
			return false;
		}
...
void UMicrophoneSpeakComponent::RPCClientTransmitVoiceData_Implementation(TArray<uint8> const &data, int32 sampleRate, int32 numchannels, int32 PCMSize, int32 opusFramePerSec, bool _isGlobal, int _radioChannel, bool _useRange, float _maxRange) {
...
void UMicrophoneSpeakComponent::payloadReceivedVoiceData(TArray<uint8> const &dataEncoded, int32 sampleRate, int32 numchannels, int32 PCMSize, int32 opusFramePerSec) {
	...
	if (!wasInitAudioResources) {
		...
		if (!resInit) {
			UE_LOG(LogTemp, Warning, TEXT("UMicrophoneSpeakComponent::payloadReceivedVoiceData wasn't init default"));
			return;
		}
	}
	PlayAudioVoice();
	...
	int32 sizeBufferVolumeGain = bufferRPCdecodedData.Num();
	...
	if (!APlayerVoiceChatActor::muteAll && !isMutedLocalSetting && VoiceCaptureSoundWaveProcedural != NULL) {
...
void UMicrophoneSpeakComponent::transmitDataToOtherAndLocal(uint8 *pointerBufferPCMMic, int32 uint8sizeBufferPCMMic) {
	...
	if (volumeGain < 0.99f || volumeGain > 1.01f) {
		for (int i = 0; i < sizeBufferVolumeGain / 2; i++) {
			...
			//UE_LOG(LogTemp, Warning, TEXT("pcm16DataTestHear[i] %d valGain %d"), pcm16DataVolume[i], valGain);
		}
	}
	...
	latestVolume = sumPCMVolume;
	timerResetRuntimeVolume = 0.0f;
	//UE_LOG(LogTemp, Warning, TEXT("sumPCMVolume mean %d"), sumPCMVolume);
	...
	if (encodedDataSize > 0) {
		AsyncTask(ENamedThreads::GameThread, [=]() {
			if (this != NULL && this->IsValidLowLevel() && IsValid(this)) {
				...
				if (!APlayerVoiceChatActor::muteAll && shouldHearMyOwnVoiceLocalSetting && VoiceCaptureSoundWaveProcedural != NULL) {
...
bool UMicrophoneSpeakComponent::OpusInit(int32 InSampleRate, int32 InNumChannels, int32 opusFramePerSec)
{
	...
	if (InSampleRate != 8000 &&
		...
		InSampleRate != 48000)
	{
		UE_LOG(LogTemp, Error, TEXT("opus Voice encoder doesn't support %d hz"), InSampleRate);
		return false;
	}
	...
	if (opusFramePerSec != 400 &&
		...
		opusFramePerSec != 25)
	{
		UE_LOG(LogTemp, Error, TEXT("opus Voice encoder doesn't support opusFramePerSec %d per sec"), opusFramePerSec);
		return false;
	}
...
bool UMicrophoneSpeakComponent::OpusDecoderInit(int32 InSampleRate, int32 InNumChannels, int32 opusFramePerSec)
{
	...
	if (opusFramePerSec != 400 &&
		...
		opusFramePerSec != 25)
	{
		UE_LOG(LogTemp, Error, TEXT("opus Voice decoder doesn't support opusFramePerSec %d per sec"), opusFramePerSec);
		return false;
	}
